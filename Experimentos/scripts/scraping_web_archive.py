    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                log_error(
                    f"[Selenium] Erro ao baixar {url}: {str(e)}", index, url)
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    log_error(
                        f"[Wayback] Erro ao processar JSON: {response.text}", index, url)
            elif attempt == retries - 1:
                log_error(
                    f"[Wayback] Erro ao acessar API: Status {response.status_code}", index, url)
...
def main():
    ...
    if not os.path.exists(path_output):
        pd.DataFrame(columns=['expanded_url', 'html_content', 'from WebArchive']).to_csv(
    ...
    try:
        for index, url in tqdm(enumerate(dataset['expanded_url']), desc="Processando URLs"):
            try:
                html_content, from_web_archive = get_html_content(
                    url, driver, index=index)

                if html_content is None:
                    log_error(
                        f"[Main] Falha em todos os métodos para: {url}", index, url)
                    continue
                ...
                log_error(
                    f"[Main] Erro crítico com {url}: {str(e)}", index, url)
    finally:
        driver.quit()
